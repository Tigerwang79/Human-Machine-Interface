...
public class Jeu {
	...
	public void menu(int choix) throws IOException {
		switch (choix){
			case 1 :
				preparation();
                                cont.changeVue(0);
			...
			case 2 :
                                cont.changeVue(11);
                                this.cont.actionaPropos();
                                this.cont.clearText();
                                this.cont.showString("\n\n");
                                this.cont.showString("Synopsis :\n\n");
				this.cont.showString("Vous incarnez Andrew Benett un célèbre inspecteur, votre but est la recherche de grimoires magiques.\n");
				this.cont.showString("Vous entendez parler d'un meurtre suspect, probablement du l\'un de ses grimoires.\n");
				this.cont.showString("Vous décidez de vous rendre en ville et accompagné du commissaire local vous aller mener votre enquête.\n");
				this.cont.showString("Vous vous rendez vite compte que le meurtre est lié a la magie.\n");
				this.cont.showString("\nVotre enquête vous mèneras dans plusieurs endroit clés de la ville, et a rencontré plusieurs personnes qui vous aiderons. "
                                        + "Grâce à vos connaissances vous devrez trouver la cause de la mort ainsi que le coupable. Mais attention les grimoires recèlent plein de secrets.\n");
                                this.cont.showString("\n");
				break;
			case 3:
				System.exit( 0 );
	...
	public boolean actionsJoueur(int choix){
	...
	public void parler() {
		...
		if (player.getPosition().getListePerso().size() == 0) {
			switch (player.getPosition().getId()){
				case 0 :
					this.cont.showString("Vous ne laissez jamais entrer personne dans votre bureau\n" +
							"Vous etes donc seul.");
					break;
				case 4 :
					this.cont.showString("Un agent s'occupe de proteger le scène de crime,\n" +
							"vous ne pensez pas pouvoir tirer des informations de lui.");
					break;
				default:
					this.cont.showString("Il n\'y a personne.");
			}
                        this.cont.actionJoueur();
		} else {
                        this.cont.clearButton();
			for (int i = 0; i < player.getPosition().getListePerso().size(); i++) {
				this.cont.addButtonChoixPerso("\t"+ player.getPosition().getListePerso().get(i).getNom()+"\n", i);
                                
			}
			this.cont.showString("\nChoisissez a qui vous voulez parler : ");
                }
        }
        public void parlerBis(int i){
            //clearConsole();
            this.cont.clearButton();
            Personnages perso = player.getPosition().getListePerso().get(i);
            perso.talk(i);
            this.cont.showString("");
        }
        
        
        public void parlerTer(int rep, int id){
            this.cont.clearText();
            Personnages perso = player.getPosition().getListePerso().get(id);
            perso.talkBis(rep);
            Dialogues dial = perso.getDialogue();
            dial.unlockItems();
            if( dial.nbLieu() != 0){
            ...
            if(perso.getDialogue().getIsEndOfGame()){
                this.cont.clearButton();
                this.cont.showString(perso.getNom() + " : \n");
                perso.getDialogue().affiche();
                this.cont.buttonReset();
                finJeu();
                }else{
                    if(dial.getIsEnd()){
                        this.cont.showString(perso.getNom() + " : \n");
                        ...
                        if(dial.getId()==48 || dial.getId()==47){
                            this.cont.add(0);
                            Objets obj = this.player.getPosition().getObjet(0);
                            if(!obj.getIsTaken() && obj.getCanBeFound()){
                                this.cont.add(0);
                            }
                        }
                        parlerBis(id);
	...
	public void deplacer(){
            ...
            if (player.getLieuxVisit().size() == 0) {
                    this.cont.showString("Aucun lieu(x) présent pour se déplacer");
                    this.cont.actionJoueur();
            } else {
                    for (int i = 0; i < player.getLieuxVisit().size(); i++) {
                            this.cont.add(player.getLieuxVisit().get(i).getId());
                    }
                    this.cont.showString( "\nOù souhaitez vous aller ?" );

            }
        }
        public void changePos(int choix) throws IOException{
            this.cont.clearButton();
            this.cont.clearText();
            
            
            int nbLieuVisit = this.player.getLieuxVisit().size();
            for(int i=0; i<nbLieuVisit; i++){
                if(player.getLieuxVisit().get(i).getId()==choix){
                    this.player.changePos(player.getLieuxVisit().get(i));
                    this.cont.changeVue(player.getLieuxVisit().get(i).getId());
                }
            }

            int nbObjets = this.player.getPosition().nbObjet();
            Objets obj;
            for(int i=0; i< nbObjets; i++){
                obj = this.player.getPosition().getObjet(i);
                if(!obj.getIsTaken() && obj.getCanBeFound()){
                    this.cont.add(i);
                }
            }
		...
		lieux.add(new Lieux(2, "Commissariat - morgue", "Vous descendez les escaliers pour aller a la morgue.\n" +
				"Dans cette pièce de 20 mettre carré majoritairement blanche\n" +
				"se trouve la médecin légiste Sonia Rousselle\n" +
				"à coté d'elle se trouve un cadavre, figé dans une expression d'horreur permanante\n"));
		...
		lieux.add(new Lieux(5, "Restaurant \"Bol de la Loutre\" ", "Le bâtiment de forme rectangulaire est composé d\'un unique rez-de-chaussée.\n" +
				"A l\'intérieur, se trouve une salle pouvant contenir 12 personnes. Les murs propres et lisses, démontrent la qualité du restaurant.\n" +
		...
		lieux.add(new Lieux(6, "Maison famille victime", "Le bâtiment en forme de L est composé d'un rez-de-chaussée et de deux étages,\n" +
				...
				"A l\'étage se trouve une chambre spacieuse. La chambre peut accueillir un couple, la porte se ferme avec un verrou.\n" +
		...
		objets.add(new Objets( 1, "Paire de Lunette cassé", "Une paire de lunettes grise cassées, \nvous les avez trouvé pres d'un buisson a l'ecart de la scène de crime." ));
		...
		objets.add(new Objets(5, "Codex Umbra", "Nom : Ombrage\n" +
				...
				"Andrex : Un sacrifice humain ? Je crois comprendre les raisons de la disparition de l'assistante de la medecin legiste.\n" +
...
//Medecin Legiste
		...
		dialogues.add(new Dialogues( 26, "Comment ça ?\n\n" +
				...
				"Une vois le monstre transfomer en perle vous laissez la lumiere dessus.\n" +
				"Ainsi il ne vous embetera plus.", false, false ));
		dialogues.add(new Dialogues( 27, "Merci, passez une bonne journée.", true, false ));
		dialogues.add(new Dialogues( 28, "Ma créature !\n\n" +
				"A ce moment la, le commissaire entre.\n" +
				"Il comprend vite la situation et arrête Sonia Rousselle\n" +
				"Vous en profitez pour mettre la perle en securité\n" +
				"la ou elle sera toujours soumise a la lumiere.\n"+
		...
		dialogues.add(new Dialogues(43, "Je faisais une promenade nocturne, lorsque près du pont, \n" +
				...
				"Je n'ai rien pu faire ... Yve lui a réussis à s'enfuir ...", false, false));
		dialogues.add(new Dialogues(44, "J\'ai croisé  deux personnes ce soir là, le pauvre Francois\n" +
				"et cette crapule de Yve Rinnal.", false, false));
		...
		dialogues.add(new Dialogues(78, "Bien je vais tout vous raconter\n" +
				"Il y a 20 ans, vous etes venus chercher un grimoire.\n" +
				"Je ne pense pas que vous vous souvenez de moi,\n" +
				"J'étais maire a l'époque.\n" +
				...
				"François venait de temps en temps, et au fur et a mesure nous sommes devenus amis \n" +
		...
		this.player = new Joueur(lieux.get(0));
                this.player.addController(cont);
		...
		//Affichages des infos de base
                this.cont.clearText();
		this.cont.showString("Vous incarnez Andrew Benett, un homme à l'allure tout à fait normale. Sauf que vous avez un rôle, vous êtes le gardien de la Porte. Vous possédez le pouvoir de créer une porte n\'importe où et n\'importe quand.\n"+
                                "\n" +
				...
				"Votre quête sera de regrouper tous les ouvrages magiques afin d\'éviter qu\'ils ne tombent entre de mauvaises mains désirant semer le chaos.\n");

		this.cont.showString("Vous voilà dans votre bureau.\n" +
		...
		this.cont.showString("Lors de votre enquête journalière, vous entendez parler d\'une mort très mystérieuse dans la ville de Bois-Sur-Suif.\n" +
		...
		this.cont.showString("Que souhaitez-vous faire ?\n");
	...
	public void finJeu(){
		this.cont.showString("***\t Fin Du Jeu \t***\n\n"+
                                    "***\t Credit \t***\n\n"+
                        "---Programmation\n"+
				"\tGuillaume\tPierre\n" +
				"\tValdrin\t\tSalihi\n"+
